...
router.get('/', async (req, res, next) => {
  try {
    const { page = 1, limit = 20, status, paymentMethod, search, dateFrom, dateTo } = req.query;
    ...
    const where: {
      ...
      OR?: Array<{ 
        referenceNumber?: { contains: string };
...
router.post('/', async (req: AuthenticatedRequest, res, next) => {
  try {
    ...
    const totalCost = data.selectedTests.reduce((total, selectedTest) => {
      const test = tests.find((t: any) => t.id === selectedTest.testId);
